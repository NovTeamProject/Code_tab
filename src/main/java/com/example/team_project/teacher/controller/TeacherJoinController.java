...
public class TeacherJoinController extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    // 성공 or 실패
    if (result == 1) {
      JSFunction.alertLocation(resp, "회원가입에 성공했습니다.",req.getContextPath() + "/membership/views/login.jsp");
    } else { //글쓰기 실패
      JSFunction.alertLocation(resp, "회원가입에 실패했습니다.",req.getContextPath() +"/membership/views/join.jsp");
    }
  }
}
